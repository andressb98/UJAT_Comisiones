<script lang="ts">
	...
	$: filteredUnidades = unidades.filter((u) =>
		u.descripcion.toLowerCase().includes(searchUnidad.toLowerCase())
	);
	...
	function selectOption(field, id, label, searchVar) {
		...
		if (field == 'unidadId') searchUnidad = label;
		activeDropdown = null;
	}
</script>
						...
						<div class="dropdown is-fullwidth {activeDropdown === 'docente' ? 'is-active' : ''}">
							<div class="dropdown-trigger">
								<input
									...
									on:focus={() => (activeDropdown = 'docente')}
								/>
							</div>
								...
								<div class="dropdown-content" style="max-height: 200px; overflow-y: auto;">
									{#each filteredDocentes as d}
										<button
											...
											on:click={() =>
												selectOption('docenteId', d.id, `${d.nombreProf} ${d.apePatProf}`)}
										>
											{d.nombreProf}
											{d.apePatProf}
										</button>
							...
							<div class="dropdown-menu" role="menu">
								<div class="dropdown-content">
									{#each filteredTipos as t}
										<button
											class="dropdown-item is-button w-100"
											on:click={() => selectOption('tipoComisionId', t.id, t.nombre)}
										>
											{t.nombre}
										</button>
									{/each}
							...
							<div class="dropdown-menu" role="menu">
								<div class="dropdown-content">
									{#each filteredLugares as l}
										<button
											class="dropdown-item is-button w-100"
											on:click={() => selectOption('lugarId', l.id, l.descripcion)}
										>
											{l.descripcion}
										</button>
									{/each}
